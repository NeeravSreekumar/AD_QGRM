...
def loss_function(p_out, p_obs, method="kl-divergence"):
...
def laplace_smooth(distribution, ncells):
    alpha = 1
...
class Model(quantum_circuit):
    def __init__(self, ncells, genes, theta, edges, p_obs, drop_zero=True,
        ...
        info_print("The QuantumGRN model is being initialized with "
    ...
    def _gradient_is_not_empty(self):
        if self.gradient is not None:
            info_print("Gradients for the QuantumGRN optimization "
                       "are already initialized", level="E")
    ...
    def compute_preprocessing(self):
        ...
        self.h_p_out, self.h_N_out = laplace_smooth(self.p_out, self.ncells)
        self.h_p_obs, self.h_N_obs = laplace_smooth(self.p_obs, self.ncells)
    ...
    def compute_gradient(self, ratio=1):
        ...
        if self.method == "kl-divergence":
            ...
            raise AttributeError("The {method} method is not "
                                 ...
                                 "modeling".format(method=self.method))
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            loss = loss_function(self.h_p_out, self.h_p_obs, self.method)
